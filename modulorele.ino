...
void loop() {
  ...
  if (stringComplete) {
    // clear the string:
    if(inputString.startsWith("acender a luz")){
      //digitalWrite(pinoLampada, !digitalRead(LED_BUILTIN));// toggle
      digitalWrite(pinoLampada, HIGH);
      Serial.print("Assistente Luz ligada.");
    }else if(inputString.startsWith("apagar a luz")){
      digitalWrite(pinoLampada, LOW);
      Serial.print("Assistente Luz desligada.");
    }
